...
class PersonalController extends Controller
{
    ...
    public function update(Request $request, User $usuario)
    {
        ...
        if (! empty($data['name']))     $usuario->name     = $data['name'];
        if (! empty($data['username'])) $usuario->username = $data['username'];
        if (array_key_exists('is_super_admin', $data)) $usuario->is_super_admin = $data['is_super_admin'] ?? false;
        if (array_key_exists('activo', $data))          $usuario->activo         = $data['activo'] ?? true;
