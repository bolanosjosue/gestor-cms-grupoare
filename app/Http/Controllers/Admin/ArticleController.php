...
class ArticleController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('admin.articles.index')->with('status', 'Artículo creado.');
    }
    ...
    public function destroy(Article $article)
    {
        ...
        return redirect()->route('admin.articles.index')->with('status', 'Artículo eliminado.');
    }
    ...
    public function publish(Article $article)
    {
        ...
        return redirect()->route('admin.articles.index')->with('status', 'Artículo publicado.');
    }
    ...
    public function unpublish(Article $article)
    {
        ...
        return redirect()->route('admin.articles.index')->with('status', 'Artículo pasado a borrador.');
    }
    ...
    protected function fillArticle(Article $article, Request $request, array $data, ?string $oldSlug): void
    {
        ...
        // Store per-article recommended ids (exclude self if present)
        $recommended = $request->input('recommended_article_ids', []);
        if (!is_array($recommended)) {
            $recommended = [];
        }

        // ensure we don't include the article itself
        if ($article->exists) {
            $recommended = array_values(array_filter($recommended, function ($id) use ($article) {
                return (int) $id !== (int) $article->id;
            }));
        }
        ...
        $article->recommended_article_ids = $recommended;
    }
