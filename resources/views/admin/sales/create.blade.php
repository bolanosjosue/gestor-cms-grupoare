...
@section('title', 'Nueva publicación de venta')
    ...
    <form method="POST" action="{{ route('admin.sales.store') }}" enctype="multipart/form-data">
        ...
        <div class="form-section">
            ...
            <div class="form-grid-3">
                <div>
                    ...
                    <input id="code" type="text" name="code" value="{{ old('code') }}" placeholder="Ej: BUF-001" required>
                ...
                <div>
                    ...
                    <input id="price_crc" type="number" name="price_crc" min="0" step="1" value="{{ old('price_crc') }}" placeholder="0" required>
        ...
        <div class="form-section">
            ...
            <div class="form-grid-2">
                <div>
                    ...
                    <input id="age_years" type="number" name="age_years" min="0" max="40" value="{{ old('age_years') }}">
                    @error('age_years')<div class="field-error">{{ $message }}</div>@enderror
                </div>
                <div>
                    <label for="weight_kg">Peso (kg)</label>
                    <input id="weight_kg" type="number" name="weight_kg" min="0" step="0.01" value="{{ old('weight_kg') }}">
                ...
                <div>
                    ...
                    <select id="breed_id" name="breed_id">
                        ...
                        @foreach($breeds as $breed)
                            <option value="{{ $breed->id }}" {{ (string) old('breed_id') === (string) $breed->id ? 'selected' : '' }}>{{ $breed->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="sex" name="sex" required>
                        <option value="female" {{ old('sex', 'female') === 'female' ? 'selected' : '' }}>Hembra</option>
            ...
            <div class="form-grid-2">
                <div>
                    ...
                    <select id="father_breed_id" name="father_breed_id">
                        ...
                        @foreach($breeds as $breed)
                            <option value="{{ $breed->id }}" {{ (string) old('father_breed_id') === (string) $breed->id ? 'selected' : '' }}>{{ $breed->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="mother_breed_id" name="mother_breed_id">
                        ...
                        @foreach($breeds as $breed)
                            <option value="{{ $breed->id }}" {{ (string) old('mother_breed_id') === (string) $breed->id ? 'selected' : '' }}>{{ $breed->name }}</option>
                        @endforeach
                    </select>
            ...
            <div class="form-grid-2">
                <div>
                    ...
                    <select id="reproductive_status" name="reproductive_status" required>
                        <option value="empty" {{ old('reproductive_status', 'empty') === 'empty' ? 'selected' : '' }}>Vacía</option>
                        <option value="pregnant" {{ old('reproductive_status') === 'pregnant' ? 'selected' : '' }}>Preñada</option>
                        <option value="producing" {{ old('reproductive_status') === 'producing' ? 'selected' : '' }}>En producción</option>
                    </select>
                    @error('reproductive_status')<div class="field-error">{{ $message }}</div>@enderror
                </div>
                <div>
                    <label for="gestation_months">Meses de Gestación</label>
                    <input id="gestation_months" type="number" name="gestation_months" min="0" max="12" value="{{ old('gestation_months') }}" placeholder="Si aplica">
                    @error('gestation_months')<div class="field-error">{{ $message }}</div>@enderror
                </div>
                <div>
                    <label for="births_count">Número de Partos</label>
                    <input id="births_count" type="number" name="births_count" min="0" max="30" value="{{ old('births_count') }}">
                ...
                <div>
                    ...
                    <input id="milk_production" type="number" name="milk_production" min="0" step="0.01" value="{{ old('milk_production') }}" placeholder="Si aplica">
            ...
            <div class="form-grid-3">
                <div>
                    ...
                    <select name="vaccines_up_to_date" id="vaccines_up_to_date_check">
                        <option value="0" {{ !old('vaccines_up_to_date') ? 'selected' : '' }}>No</option>
                        <option value="1" {{ old('vaccines_up_to_date') ? 'selected' : '' }}>Sí</option>
                    </select>
                ...
                <div>
                    ...
                    <select id="feeding_type" name="feeding_type" required>
                        <option value="grazing" {{ old('feeding_type', 'grazing') === 'grazing' ? 'selected' : '' }}>Pastoreo</option>
                        <option value="supplement" {{ old('feeding_type') === 'supplement' ? 'selected' : '' }}>Suplemento</option>
                        <option value="mixed" {{ old('feeding_type') === 'mixed' ? 'selected' : '' }}>Mixto</option>
                    </select>
                ...
                <div>
                    ...
                    <select id="animal_condition" name="animal_condition" required>
                        <option value="excellent" {{ old('animal_condition') === 'excellent' ? 'selected' : '' }}>Excelente</option>
                        <option value="good" {{ old('animal_condition', 'good') === 'good' ? 'selected' : '' }}>Buena</option>
                        <option value="regular" {{ old('animal_condition') === 'regular' ? 'selected' : '' }}>Regular</option>
                    </select>
        ...
        <div class="form-section">
            ...
            <div>
                ...
                <input id="phone" type="text" name="phone" value="{{ old('phone', '+58') }}" required placeholder="+584121234567">
        ...
        <div class="mt-3 text-right">
            <button class="btn btn-primary" type="submit">Publicar venta</button>
        </div>
    </form>
@endsection
